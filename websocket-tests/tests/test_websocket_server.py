...
class WebSocketTestSuite:
            ...
            async with websockets.connect(self.server_url) as websocket:
                ...
                try:
                    ...
                    return TestResult(
                        ...
                        duration=time.time() - start_time,
                        details={
                            "invalid_sent": "invalid json",
                            "valid_response": json.loads(response),
                        },
                    )
                except asyncio.TimeoutError:
                    return TestResult(
    ...
    async def test_connection_stress(self) -> TestResult:
        ...
        try:
            ...
            self.log(
                f"✅ Successfully handled {max_connections} concurrent connections",
                "SUCCESS",
            )
            return TestResult(
                test_name="Connection Stress Test",
...
async def test_websocket_connection_basic():
    """Pytest test for basic WebSocket connection"""
    test_suite = WebSocketTestSuite()
    result = await test_suite.test_connection_basic()
    assert result.status == "PASS", f"Basic connection failed: {result.error_message}"


@pytest.mark.asyncio
async def test_websocket_chat_message():
    """Pytest test for chat message handling"""
    test_suite = WebSocketTestSuite()
    result = await test_suite.test_chat_message()
    assert result.status == "PASS", f"Chat message test failed: {result.error_message}"


@pytest.mark.asyncio
async def test_websocket_stroke_message():
    """Pytest test for stroke message handling"""
    test_suite = WebSocketTestSuite()
    result = await test_suite.test_stroke_message()
    assert (
        result.status == "PASS"
    ), f"Stroke message test failed: {result.error_message}"


@pytest.mark.asyncio
async def test_websocket_invalid_message():
    """Pytest test for invalid message handling"""
    test_suite = WebSocketTestSuite()
    result = await test_suite.test_invalid_message()
    assert (
        result.status == "PASS"
    ), f"Invalid message test failed: {result.error_message}"
...
async def main():
    ...
    try:
        async with websockets.connect("ws://localhost:9000") as websocket:
            console.print(
                "✅ Server is running and accepting connections", style="green"
            )
    except Exception as e:
        console.print(f"❌ Cannot connect to server: {e}", style="red")
        console.print(
            "Please start the WebSocket server first: ./websocket-server/build/server",
            style="yellow",
        )
        return
