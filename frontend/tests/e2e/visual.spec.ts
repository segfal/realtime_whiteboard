...
test.describe('Visual Regression Tests', () => {
  ...
  test('should match different tool states', async ({ page }) => {
    ...
    await expect(page.locator('.toolbar')).toHaveScreenshot('toolbar-rectangle-selected.png');
    
    // Test eraser tool
    await page.click('button[title="Eraser"]');
    await expect(page.locator('.toolbar')).toHaveScreenshot('toolbar-eraser-selected.png');
  });
}); 