...
class DrawingEngineBridge {
  ...
  async loadWASM(): Promise<void> {
    try {
      ...
      while (!DrawingEngineModule && attempts < maxAttempts) {
        // @ts-expect-error - DrawingEngineModule is dynamically loaded from WASM
        DrawingEngineModule =
          (window as unknown).DrawingEngineModule ||
          (globalThis as unknown).DrawingEngineModule;
      ...
      if (!DrawingEngineModule) {
        throw new Error(
          "DrawingEngineModule not found after waiting. Make sure the WASM script is loaded.",
        );
      }
      ...
      console.log(
        ...
        Object.getOwnPropertyNames(this.module),
      );
      ...
      const hasMethod = (obj: unknown, methodName: string) => {
        return (
      ...
      console.log(
        "- addPointToStroke:",
        hasMethod(this.engine, "addPointToStroke"),
      );
  ...
  addStroke(stroke: WASMStroke): void {
    ...
    const strokeShape = new StrokeShape(
      stroke.color,
      stroke.thickness,
      pointsVec,
    );
  ...
  getStrokes(): WASMStroke[] {
    ...
    if (
      typeof strokesVec.size === "function" &&
      typeof strokesVec.get === "function"
    ) {
      const arr = [];
