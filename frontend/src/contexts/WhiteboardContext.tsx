...
export const WhiteboardProvider: React.FC<WhiteboardProviderProps> = ({
  ...
  const connectWebSocket = useCallback(async () => {
    try {
      ...
      if (!roomId || roomId === "") {
        console.error("No room ID found in URL:", window.location.pathname);
        return;
      }

      console.log("Connecting to WebSocket for room:", roomId);
      const goWebSocketService = new GoWebSocketService("ws://localhost:8080");
      ...
      goWebSocketService.setConnectHandler(() => {
        ...
        goWebSocketService.joinRoom(roomId);
      });
      ...
      // Connect with the room ID
      await goWebSocketService.connect(roomId);
    } catch (error) {
    ...
    (message: GoWebSocketMessage) => {
      ...
      switch (message.type) {
        ...
        case "stroke": {
          ...
          // The username check is now reliable because the backend adds it.
      ...
      try {
        ...
        switch (message.type) {
          ...
          case "stroke:erase": {
            ...
            if (
              strokeIndex >= 0 &&
              strokeIndex < wasmEngine.getStrokes().length
            ) {
