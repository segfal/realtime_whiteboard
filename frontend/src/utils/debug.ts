...
export class ToolDebugger {
  private static toolHistory: Array<{
    from: string;
    to: string;
    timestamp: number;
  }> = [];
  ...
  static logWASMLoad(module: WASMModule) {
    if (process.env.NODE_ENV === "development") {
      console.log("[WASM_LOAD] Module loaded:", {
        ...
        methods: Object.getOwnPropertyNames(module).filter(
...
export class PerformanceTracker {
  private static measurements: Map<string, { start: number; end?: number }> =
    new Map();
